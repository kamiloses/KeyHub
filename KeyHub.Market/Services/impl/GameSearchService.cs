...
public class GameSearchService : IGameSearchService
{
    ...
    public async Task<(List<GameDto> Games, int TotalGames)> GetSearchedGames(string? title, GameSort sortBy,
        ...
        int pageSize)
    {
        try
        {
            IQueryable<Game> gamesQuery =
                GetFilteredAndSortedGames(title, sortBy, platforms, genres, minPrice, maxPrice);
    ...
    public IQueryable<Game> GetFilteredAndSortedGames(string? title, GameSort sortBy, Platform[]? platforms,
        Genre[]? genres,
